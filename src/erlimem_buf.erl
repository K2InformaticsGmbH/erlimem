...
% if finished restart
get_prev_rows(#buffer{state=finished} = Buf, MaxRows) -> get_prev_rows(Buf#buffer{state=started}, MaxRows);
...
get_next_rows(#buffer{state=finished} = Buf, _) -> {{[],undefined,undefined}, Buf};
